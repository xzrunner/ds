...
ds_hash_create(int capacity, int hash_size, float rehash_weight,
	...
	struct ds_hash* hash = (struct ds_hash*)malloc(sizeof(struct ds_hash));
	if (!hash) {
		return NULL;
	}
...
static inline void
_enlarge_freelist(struct ds_hash* hash) {
	struct changeable* old = hash->c;

//	ds_info(" [hash] node resize %d -> %d", old->buf_sz, old->buf_sz * 2);
...
_new_hash_node(struct ds_hash* hash) {
	if (hash->c->freelist) {
...
//	ds_info(" [hash] rehash %d -> %d", old->hash_sz, new_hash_sz);
	...
	for (int i = 0; i < old_buf_sz; ++i) {
		...
		ds_hash_insert(hash, keys[i], vals[i], true);
	}
}
...
ds_hash_insert(struct ds_hash* hash, void* key, void* val, bool force) {
	...
	if (!force) {
		void* val = ds_hash_query(hash, key);
		if (val) {
			return;
		}
	}
